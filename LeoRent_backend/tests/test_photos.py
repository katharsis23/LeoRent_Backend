...
class TestPhotoDownload:
    ...
    async def test_download_photo_with_spaces(self, mock_download, client):
        ...
        response = client.get(
            "/photos/download/Screenshot%20from%202026-02-17%2012-14-56.png")

        assert response.status_code == 200
        assert response.content == photo_bytes

    @patch("src.leorent_backend.services.backblaze_service.backblaze_service.download_photo")
    async def test_download_photo_with_nested_path(
        ...
        response = client.get(
            "/photos/download/apartments/456/living-room/photo-1.jpg")

        assert response.status_code == 200
        assert response.content == photo_bytes
        # Перевіри що file_key передано правильно
        mock_download.assert_called_once_with(
            "apartments/456/living-room/photo-1.jpg")
...
class TestPhotoDelete:
    ...
    async def test_delete_photo_success(self, mock_delete, client):
        ...
        ) or "успішно" in data["message"].lower()
    ...
    async def test_delete_photo_failure(self, mock_delete, client):
        ...
        assert "не вдалось" in response.json()["detail"].lower(
        ) or "fail" in response.json()["detail"].lower()
    ...
    async def test_delete_photo_with_spaces(self, mock_delete, client):
        ...
        response = client.delete(
            "/photos/delete/Screenshot%20from%202026-02-17%2012-14-56.png")
...
class TestBackblazeServiceUnit:
    ...
    async def test_upload_photo_from_url_unit(
            self, mock_http_client, mock_s3_client):
